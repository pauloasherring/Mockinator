...
class Function(_GenericDeclaration):
    ...
    def get_return_type(self):
        ret = self.return_type.full_name() + ' ' if (isinstance(self.return_type, Type)) else ''
        return ret
         
    def get_argument_signature_types_names(self):
        ret = '('
        cnt = 0
        paramCount = 0
            ...
            if isinstance(param,Parameter):
                if param.name is not None:
                    if param.type is not None:
                        if len(param.type.modifiers) > 0:
                            for item in param.type.modifiers:
                                ret += str(item) + ' '
                        if len(param.type.name) > 0:
                            ret += param.type.name + ' '
                        if param.type.reference > 0:
                            ret += ' & '
                        if param.type.pointer > 0:
                            ret += ' * '
                        if len(param.name) > 0:
                            ret += param.name
                        if param.type.array == True:
                            ret += '[] '
                        cnt += 1
                        if (cnt < paramCount):
                            ret += ', '
    ...
    def get_argument_signature_names(self):        
        ret = '('
        cnt = 0
        paramCount = 0
        for param in self.parameters:
    ...
    def get_argument_signature_types(self):        
        ret = '('
        cnt = 0
        paramCount = 0
        for param in self.parameters:
...
class Type(_GenericDeclaration):
    ...
    def full_name(self):
        ret = ''
        #if (isinstance(self.templated_types,None.NoneType)):
        ...
        #else:
        #    #raise('Error! Templated types not supported')
    def __str__(self):
        prefix = ''
        if self.modifiers:
